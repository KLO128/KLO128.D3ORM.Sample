...
{
    public class D3UserAndRolesByEmailQuery
    {
        ...
        public static ISpecification<User> GetSingleton(ID3Context d3Context)
        {
            ...
            {
                that = D3Specification.Create<User>(d3Context).CompareFormat(x => x.Email, ComparisonOp.EQUALS, 0, LogicalOp.NONE).And(D3Specification.Create<UserRole>(d3Context));
