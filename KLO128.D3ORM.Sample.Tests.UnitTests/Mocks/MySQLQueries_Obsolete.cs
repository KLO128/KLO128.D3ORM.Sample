...
FROM `team` `team`
INNER JOIN `team_player` `team_player` ON `team`.`team_id` = `team_player`.`team_id`
INNER JOIN `user` `user` ON `team_player`.`player_id` = `user`.`user_id`
...
FROM `team` `team`
INNER JOIN `tournament_team` `tournament_team` ON `team`.`team_id` = `tournament_team`.`team_id`
INNER JOIN `tournament_team_stat` `tournament_team_stat` ON `tournament_team`.`tournament_team_id` = `tournament_team_stat`.`tournament_team_id`
...
FROM `user` `user`
INNER JOIN `user_role` `user_role` ON `user`.`user_id` = `user_role`.`user_id`
...
INNER JOIN `tournament_team` `tournament_team` ON `tournament`.`tournament_id` = `tournament_team`.`tournament_id`
...
FROM `match` `match`
INNER JOIN `team` `team` ON `match`.`home_team_id` = `team`.`team_id`
INNER JOIN `team` `x_team` ON `match`.`away_team_id` = `x_team`.`team_id`
...
INNER JOIN `user` `user` ON `tournament_player_stat`.`player_id` = `user`.`user_id`
INNER JOIN `tournament` `tournament` ON `tournament_player_stat`.`tournament_id` = `tournament`.`tournament_id`
...
WHERE
	( ( `team`.`name` = 'Team1' OR `team`.`name` = 'Team2' OR `team`.`name` = 'Team5' ) OR ( `x_team`.`name` = 'Team1' OR `x_team`.`name` = 'Team2' OR `x_team`.`name` = 'Team5' ) ) OR ( ( `team`.`name` = 'Team3' OR `team`.`name` = 'Team4' OR `x_team`.`name` = 'Team3' OR `x_team`.`name` = 'Team4' ) )
...
INNER JOIN `team` `team` ON `match`.`home_team_id` = `team`.`team_id`
INNER JOIN `team` `x_team` ON `match`.`away_team_id` = `x_team`.`team_id`
INNER JOIN `team_player` `x_team_player` ON `x_team`.`team_id` = `x_team_player`.`team_id`
INNER JOIN `user` `x_user` ON `x_team_player`.`player_id` = `x_user`.`user_id`
...
INNER JOIN `team` `team` ON `match`.`home_team_id` = `team`.`team_id`
INNER JOIN `team` `x_team` ON `match`.`away_team_id` = `x_team`.`team_id`
INNER JOIN `tournament_team` `x_tournament_team` ON `x_team`.`team_id` = `x_tournament_team`.`team_id`
INNER JOIN `tournament_team_stat` `x_tournament_team_stat` ON `x_tournament_team`.`tournament_team_id` = `x_tournament_team_stat`.`tournament_team_id`
...
"
            ...
            {
                Constants.ExtendedAllPossibleRootsSimpleStrictConditionExpensiveQuery, // Expensive LEFT JOIN on top
				@"SELECT DISTINCT
`playoff_round_couple`.`playoff_round_couple_id` '.PlayoffRoundCoupleId'
	, `playoff_round_couple`.`tournament_team1_id` '.TournamentTeam1Id'
	, `playoff_round_couple`.`tournament_team2_id` '.TournamentTeam2Id'
...
WHERE
	`playoff_round_couple`.`playoff_round` = 1 AND ( ( ( `team`.`name` = 'Team1' OR `team`.`name` = 'Team2' OR `team`.`name` = 'Team5' ) AND ( `x_team`.`name` = 'Team1' OR `x_team`.`name` = 'Team2' OR `x_team`.`name` = 'Team5' ) ) OR ( ( `team`.`name` = 'Team3' OR `team`.`name` = 'Team4' OR `x_team`.`name` = 'Team3' OR `x_team`.`name` = 'Team4' ) ) )
ORDER BY `match`.`tournament_id` DESC, `team`.`name` DESC, `x_team`.`name` DESC, `tournament`.`name`, `match`.`match_id`, `match_set_score`.`set_order`
;
...
WHERE
	( `tournament`.`tournament_id` IN  (
		...
		INNER JOIN `tournament_team` `xxx_tournament_team` ON `xxx_tournament`.`tournament_id` = `xxx_tournament_team`.`tournament_id`
		INNER JOIN `team` `xxx_team` ON `xxx_tournament_team`.`team_id` = `xxx_team`.`team_id`
...
ORDER BY `tournament`.`tournament_id` DESC, `team`.`name` DESC, `x_team`.`name` DESC, `tournament`.`name`
;

-- TournamentTeam1(TournamentTeam).TournamentTeamId,TournamentTeamStats[TournamentTeamStat] ...TournamentTeamStats[TournamentTeamStat].TournamentTeamId
SELECT DISTINCT
`tournament_team_stat`.`tournament_team_stat_id` '.TournamentTeamStatId'
	, `tournament_team_stat`.`tournament_team_id` '.TournamentTeamId'
	, `tournament_team_stat`.`tournament_phase` '.TournamentPhase'
...
-- TournamentTeam2(TournamentTeam).Team.TeamId,TeamPlayers[TeamPlayer] ...TeamPlayers[TeamPlayer].TeamId
SELECT DISTINCT
`x_team_player`.`team_player_id` '.TeamPlayerId'
	, `x_team_player`.`team_id` '.TeamId'
	, `x_team_player`.`player_id` '.PlayerId'
...
-- TournamentTeam2(TournamentTeam).TournamentTeamId,TournamentTeamStats[TournamentTeamStat] ...TournamentTeamStats[TournamentTeamStat].TournamentTeamId
SELECT DISTINCT
`x_tournament_team_stat`.`tournament_team_stat_id` '.TournamentTeamStatId'
	, `x_tournament_team_stat`.`tournament_team_id` '.TournamentTeamId'
	, `x_tournament_team_stat`.`tournament_phase` '.TournamentPhase'
	, `x_tournament_team_stat`.`phase_points` '.PhasePoints'
	, `x_tournament_team_stat`.`wins` '.Wins'
	, `x_tournament_team_stat`.`losts` '.Losts'
	, `x_tournament_team_stat`.`ties` '.Ties'
	, `x_tournament_team_stat`.`sets_won` '.SetsWon'
	, `x_tournament_team_stat`.`sets_lost` '.SetsLost'
	, `x_tournament_team_stat`.`score_plus` '.ScorePlus'
	, `x_tournament_team_stat`.`score_minus` '.ScoreMinus'
	, `x_tournament_team_stat`.`last_change` '.LastChange'
	, `x_tournament_team_stat`.`changed_by` '.ChangedBy'
FROM `playoff_round_couple` `playoff_round_couple`
INNER JOIN `tournament_team` `tournament_team` ON `playoff_round_couple`.`tournament_team1_id` = `tournament_team`.`tournament_team_id`
INNER JOIN `tournament` `tournament` ON `tournament_team`.`tournament_id` = `tournament`.`tournament_id`
INNER JOIN `team` `team` ON `tournament_team`.`team_id` = `team`.`team_id`
...
-- .PlayoffRoundCoupleId,Matches[Match] ...Matches[Match].PlayoffRoundCoupleId
SELECT DISTINCT
`match`.`match_id` '.MatchId'
	, `match`.`home_team_id` '.HomeTeamId'
	, `match`.`away_team_id` '.AwayTeamId'
...
ORDER BY `match`.`match_id`, `match_set_score`.`set_order`
;
"
            },
            {
...
WHERE
	`tournament`.`tournament_id` <= {0:TournamentTeam1(TournamentTeam).Tournament.TournamentId} AND `tournament`.`tournament_id` >= {1:TournamentTeam1(TournamentTeam).Tournament.TournamentId} AND ( ( `tournament`.`tournament_id` IN  (
		SELECT DISTINCT
		`xxx_tournament`.`tournament_id`
		FROM `tournament` `xxx_tournament`
		INNER JOIN `tournament_team` `xxx_tournament_team` ON `xxx_tournament`.`tournament_id` = `xxx_tournament_team`.`tournament_id`
...
WHERE
	`team`.`team_id` NOT IN  (
		...
		`xxxx_team`.`team_id`
		FROM `team` `xxxx_team`
		INNER JOIN `tournament_team` `xxxx_tournament_team` ON `xxxx_team`.`team_id` = `xxxx_tournament_team`.`team_id`
		INNER JOIN `tournament` `xxxx_tournament` ON `xxxx_tournament_team`.`tournament_id` = `xxxx_tournament`.`tournament_id`
		...
		`xxx_team`.`team_id`
		FROM `team` `xxx_team`
		INNER JOIN `team_player` `xxx_team_player` ON `xxx_team`.`team_id` = `xxx_team_player`.`team_id`
		INNER JOIN `user` `xxx_user` ON `xxx_team_player`.`player_id` = `xxx_user`.`user_id`
